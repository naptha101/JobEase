        ...
        if(profile){
            await cloudinary.uploader.upload(profile.tempFilePath).then(async (e)=>{
               ...
               const data=await SendMail({email,mailType:"verify",UserId:user._id})
                ...
                //res.status(200).cookie("token", token, { expires: new Date(Date.now() + 24 * 60 * 60 * 1000 * 5), httpOnly: true }).json({ status: true, message: "User Registered", user });
           if(data){
...
const user=await User.findOne({verifyToken: token,
 ...
 return res.status(200).cookie("token", token1, { expires: new Date(Date.now() + 24 * 60 * 60 * 1000 * 5), httpOnly: true }).json({ status: true, message: "User Registered and Verified", user });
...
res.status(200).cookie("token",token, { expires: new Date(Date.now() + 24 * 60 * 60 * 1000 * 5), httpOnly: true }).json({ status: true, message: "User Login", existingUser });
...
res.status(200).cookie("token","", { expires: new Date(Date.now()), httpOnly: true }).json({ status: true, message: "User LogOut"});
    }
    catch(err){
        next(err);
    }
